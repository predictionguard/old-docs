...
### No Context Example
This example demonstrates how to provide automated responses to customer queries using the Prediction Guard Chat API.
...
7. **Test the Response:** Ensure that the system works as intended.
8. **Map Response to Custom Field:** Link the API response to the Bot_Response1 custom field.
9. **Create Response Message Block:** Set up a block in ManyChat to relay the response to the user.
10. **Provide Additional Options:** Include options for users to ask new questions, speak to an agent, or close the conversation.

### Include Conversation Context Example
This advanced example includes the context of previous conversations in the responses.

#### Prerequisites
- Same as the No Context Example
- AWS Lambda Function accessible via the internet

#### Steps
1. **Follow Basic Example Steps:** Implement steps 1-6 from the no context example.
2. **Use Dynamic Content:** Replace the External Request Action Block with a “Get Dynamic Content” block.
3. **Send Data to Lambda Function:** Configure the block to send user data to your Lambda function.
4. **Lambda Function Goals:**
   - Parse incoming request
   - Extract custom fields and conversation history
   - Prepare messages for the Prediction Guard API request
